function istit(cfg) {
  ...
  this.linesToGet = 0;
  this.lastCounDown = 0;
  (this.numTicks = 0), (this.numDraws = 0);
  this.volume = this.defVolume;
  (this.lastWeight = 0), (this.lastWeightTime = 0);
  (this.gridWeightHistory = []), (this.messages = []);
  this.safetyTimes = 0;
  this.nextSafetyAt = 0;
  this.placedBlocks = {};
  ...
  (this.playerName = 'Player'), (this.opponentName = 'Opponent');
  this.showingNamePrompt = false;
  (this.selfLBRowID = -1), (this.selfLBRank = -1);
...
istit.prototype.init = function(canvasID) {
...
istit.prototype.syncDefDimension = function() {
  this.defWidth = this.hTiles * this.tile + this.tile * 6 + (this.tile / 2) * 3;
...
istit.prototype.parseMiliSeconds = function(ms) {
  var x = ms / 1000;
...
istit.prototype.start = function() {
  this.reset();
  this.dropPiece();
};

istit.prototype.restart = function() {
...
istit.prototype.reset = function() {
  this.input.reset();
  this.renderer.init();
  this.ended = false;
  this.runTime = 0;
...
istit.prototype.end = function(isWinner) {
  if (!this.ended) {
...
istit.prototype.inPlay = function() {
...
istit.prototype.inputIsLocked = function() {
...
istit.prototype.live = function() {
  ...
  if (this.inPlay()) {
    ...
    var now = new Date().getTime();
    if (this.runTime > this.dropAt && this.pFallingPiece.start < this.dropAt) {
      this.dropPiece();
    }
  ...
  if (this.lbIsShowing) {
    ...
    this.renderer.sysY = this.renderer.sysYDef - sysPer * this.renderer.sysYDif;
...
istit.prototype.setVolume = function(v) {
  this.volume = v;
...
istit.prototype.rowIsCleared = function(r) {
...
istit.prototype.adjustFallingHeight = function() {
  //trace('afh');
...
istit.prototype.adjustFallingHeightOffset = function() {
  this.pFallingPiece.offset++;
};

istit.prototype.adjustTime = function(dif) {
  this.time += dif;
  var elapsed = this.time - this.timeStart;
};

istit.prototype.spawnSpecial = function() {
  var num = 0,
    low = this.vTiles;
...
istit.prototype.randomPiece = function() {
  if (this.bffrPiece) {
...
istit.prototype.dropPiece = function() {
  if (!this.ended) {
...
istit.prototype.movePiece = function(d) {
...
istit.prototype.rotatePiece = function(update) {
...
istit.prototype.collides = function(xAdjust, yAdjust, rAdjust, type) {
...
istit.prototype.getFallingBlocks = function(opponent, p, t) {
...
istit.prototype.getPositionOffset = function(b, p, x, y) {
  if (p == 2) {
    if (b[1] == 1) {
      x = x + 2 - b[0];
      y = y - b[1] + b[0];
...
istit.prototype.handleGridChange = function() {
  if (this.mp.session > -1) {
    this.mp.sendState();
  }
...
istit.prototype.getHotPiece = function(lines) {
...
istit.prototype.placePiece = function() {
...
istit.prototype.getCompleteLines = function() {
  var lines = [];
  for (var v = 0; v < this.vTiles; v++) {
    var solid = true;
    for (var h = 0; h < this.hTiles; h++) {
...
istit.prototype.destroyLines = function() {
...
istit.prototype.clearLines = function(lines) {
  var str = 'line';
  if (lines.length > 1) {
    str = 'lines';
  }
    ...
    for (var s in this.pState.special) {
      for (var c = 0; c < this.hTiles; c++) {
        if (typeof this.pState.special[lines[i] + ':' + c] != 'undefined') {
          ...
          this.adjustScore(this.specialBonus, { text: 'golden block' }, false);
        }
      }
    }
  }
  ...
  this.animateTo.lineBreak = new Date().getTime() + this.animateCycle.lineBreak;
};

istit.prototype.getLines = function(num) {
  this.linesToGet += num;
...
istit.prototype.insertLines = function() {
...
istit.prototype.adjustScore = function(p, msg, giveSpeedBonus) {
...
istit.prototype.getGhostBlocks = function() {
  var ghost = [];
  var blocks = this.getFallingBlocks();
...
istit.prototype.getClosestToTopInColumn = function(c) {
...
istit.prototype.placeFallingPieceAtBottom = function() {
  var offset = this.vTiles;
  //for (var i = 0; i
  this.pFallingPiece.offset = offset;
};

istit.prototype.getLargestDrop = function() {};

istit.prototype.getPieceOffset = function(x, y) {
  var xOffset = x * this.tile;
  var yOffset = y * this.tile;
  return [xOffset, yOffset];
};

istit.prototype.setLevel = function(l) {
  this.pState.level = l;
...
istit.prototype.getLevelScores = function(mLevel) {
  var score;
  var lastScore = 0;
  this.levels[1] = 0;
...
istit.prototype.random = function(from, to) {
  return Math.floor(Math.random() * (to - from + 1) + from);
};

istit.prototype.randomWithSeed = function(mZ, mW) {
  var mZ = m_w,
    mW = m_w;
...
istit.prototype.getPieceDimension = function(t, o, d) {
...
istit.prototype.resizeForSP = function() {
...
istit.prototype.resizeForMP = function() {
...
istit.prototype.getCompoundedWeight = function() {
  var numFilled = 0,
    total = 0;
  for (var c = 0; c < this.hTiles; c++) {
...
istit.prototype.addScoreMessage = function(text, r, c) {
  var m = {
    text: text,
...
istit.prototype.toggleHold = function() {
  if (!this.holdPiece) {
...
istit.prototype.addNextPiece = function() {
...
istit.prototype.queueLeaderBoard = function(add) {
...
istit.prototype.launchLeaderBoard = function() {
